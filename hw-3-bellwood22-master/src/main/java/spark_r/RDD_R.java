...
rdd_r (reducebykey)
// setting
val conf=new SparkConf().setAppName("RDD_R").setMaster("local")
...
val counts=pairs.redcueByKey(_ + _)

// output
counts.saveAsTextFile("output/RDD_R")
 */
///////////////////////////////////////////////////////////////////////////
...
public class RDD_R {
    ...
    public static void main(String[] args){
        // spark 설정 초기화
        SparkConf conf=new SparkConf().setAppName("RDD_R").setMaster("local");
        ...
        // 팔로워 reduce
        JavaPairRDD<Integer,Integer> followerCounts=filteredFollowers.reduceByKey(new Function2<Integer,Integer,Integer>(){
            public Integer call(Integer a, Integer b) throws Exception{
                return a+b;
            }
        });

        System.out.println("Execution Plan for followerCounts:");
        System.out.println(followerCounts.toDebugString());

        // output
        followerCounts.foreach(result->System.out.println("(" + result._1 + ", " + result._2 + ")"));
        followerCounts.saveAsTextFile("output/RDD_R");
